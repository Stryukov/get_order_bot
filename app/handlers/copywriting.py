...
async def get_size(message: types.Message, state: FSMContext):
    ...
    await message.answer('Потом нажмите кнопку "Отправить заявку"',
...
async def get_voice(message: types.Message, state: FSMContext):
    if message.content_type == 'voice':
        ...
        await message.answer(
            '🔷 Пришлите голосовое сообщение или нажмите кнопку '
            '"Отправить заявку".',
            reply_markup=keyboard,
        )


async def order_finish(call: types.CallbackQuery, state: FSMContext):
    order = await state.get_data()
    user = get_user_info(call)
    await bot.send_message(config.bot.admin_group, 
                           f'🔷 <b>Новый заказ!</b> '
    ...
    await call.message.answer('Спасибо за ваш заказ! Мы отнесемся к нему креативно!')
    await state.finish()
    await call.answer()
    await cmd_welcome(call.message)
...
def register_handlers_copywriting(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(
        order_finish, Text(startswith="send_order"), state="*"
    )
